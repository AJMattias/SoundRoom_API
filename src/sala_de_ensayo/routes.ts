...
import {
  generateReporteBarChart,
  generateReportePDF,
  generateReportePieChart,
  generateReporteValoracionesPDF,
} from "../common/utils/generateReportePdf";
...
export const route = (app: Application) => {
  ...
  app.get(
    ...
    run(async (req: Request, resp: Response) => {
      ...
      if (typeof busqueda !== "string" || busqueda.trim() === "") {
        ...
        return resp
          .status(400)
          .json({ error: "El parámetro de búsqueda 'q' es obligatorio." });
      }
      console.log("Ruta: Buscando por:", busqueda);
      // Ahora pasas la cadena validada
      const salas: SalaDeEnsayoDto[] =
        await service.instance.findByName(busqueda);
  ...
  app.get(
    "/salasdeensayo/findByNamePaginated/",
    auth,
    checkPermission(["BUSCAR_SALA_DE_ENSAYO"]),
    run(async (req: Request, resp: Response) => {
      ...
      console.log(
        `Ruta: Buscando por: ${busqueda}, Page: ${page}, Limit: ${limit}`,
      );
      ...
      const salasPaginated: PaginatedResponseDto<SalaDeEnsayoDto> =
        await service.instance.findByNamePaginated(
          busqueda.trim(),
          page,
          limit,
        );
  ...
  app.get(
    ...
    run(async (req: any, resp: Response) => {
      ...
      console.log("id: ", id);
      const salas: SalaDeEnsayoDto[] = await service.instance.findSalaByOwner(
        req.user.id,
      );
  ...
  app.get(
    ...
    run(async (req: any, resp: Response) => {
      try {
        ...
        const salas = await service.instance.findSalaByOwnerPaginated(
          idOwner,
          page,
          limit,
        );
    ...
    run(async (req: Request, resp: Response) => {
      ...
      const mailOptions = {
        ...
        html:
          ...
          "El usuario " +
          dto["nombreUsuario"] +
          " ha cancelado la reserva de la sala " +
          dto["sala"] +
          " del dia " +
          dto["inicio"] +
          "</h2></td> </tr> </tbody> </table></td> </tr></tbody></table></div>",
  ...
  app.post(
    ...
    run(async (req: Request, resp: Response) => {
      ...
      const mailOptionsCliente = {
        ...
        html:
          '<div id=":pf" class="a3s aiL "><table><tbody> <tr> <td style="padding:16px 24px"> <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%"> <tbody> <tr> <td> <table role="presentation" align="center" border="0" cellspacing="0" cellpadding="0" width="auto"> <tbody> <tr> <td> <img alt="Imagen de SoundRoom" border="0" height="70" width="70" src="https://fs-01.cyberdrop.to/SoundRoom_logo-X6fFVkX9.png" style="border-radius:50%;outline:none;color:#ffffff;max-width:unset!important;text-decoration:none" class="CToWUd"></a></td> </tr> </tbody> </table></td> </tr> <tr> <td> <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%" align="center" style="max-width:396px;padding-bottom:4px;text-align:center"> <tbody> <tr> <td><h2 style="margin:0;color:#262626;font-weight:400;font-size:16px;line-height:1.5">' +
          "Usted acaba de realizar una reserva en la sala de ensayo " +
          dto["sala"] +
          " el dia " +
          dto["inicio"] +
          "</h2></td> </tr> </tbody> </table></td> </tr></tbody></table></div>",
      ...
      if (dto["receptorSala"] != "") {
        const mailOptionsDueno = {
          ...
          html:
            '<div id=":pf" class="a3s aiL "><table><tbody> <tr> <td style="padding:16px 24px"> <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%"> <tbody> <tr> <td> <table role="presentation" align="center" border="0" cellspacing="0" cellpadding="0" width="auto"> <tbody> <tr> <td> <img alt="Imagen de SoundRoom" border="0" height="70" width="70" src="https://fs-01.cyberdrop.to/SoundRoom_logo-X6fFVkX9.png" style="border-radius:50%;outline:none;color:#ffffff;max-width:unset!important;text-decoration:none" class="CToWUd"></a></td> </tr> </tbody> </table></td> </tr> <tr> <td> <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%" align="center" style="max-width:396px;padding-bottom:4px;text-align:center"> <tbody> <tr> <td><h2 style="margin:0;color:#262626;font-weight:400;font-size:16px;line-height:1.5">' +
            "Han realizado una reserva para su sala  " +
            dto["sala"] +
            " el dia " +
            dto["inicio"] +
            "</h2></td> </tr> </tbody> </table></td> </tr></tbody></table></div>",
  ...
  app.post(
    ...
    run(async (req: any, resp: Response) => {
      ...
      const perfil = await perfilService.instance.findPerfilById(
        logged.idPerfil._id,
      );
      ...
      const perfilSala = await PerfilModel.findOne({
        name: "Sala de ensayo",
      }).exec();
      ...
      if (perfil?.name === perfilSala?.name) {
        ...
        if (!userUpdate) {
          throw new Error(
            "No se puedo actualizar el usuario con el perfil Sala de Ensayo",
          );
        }
      }
      ...
      if (!salaAdded) {
        throw new Error(
          "No se puedo actualizar el usuario con la Sala de Ensayo",
        );
      }
  ...
  app.put(
    ...
    run(async (req: Request, resp: Response) => {
      ...
      const salaOriginal: SalaDeEnsayoDto =
        await service.instance.findSalaById(id);
      if (!salaOriginal) {
        return resp
          .status(404)
          .json({ status: 404, msg: "Sala de ensayo no encontrada." });
      }

      // Aquí transformas el array de objetos a un array de strings (solo los IDs)
      const incomingImageIds = dto.imagenes
        ? dto.imagenes.map((imagen: { id: string }) => imagen.id)
        : [];
      ...
      for (const imageId of deletedImageIds) {
        try {
          ...
          if (imagen) {
            ...
            console.warn(
              `[ADVERTENCIA] Imagen con ID ${imageId} no encontrada en la base de datos para eliminar.`,
            );
          }
        } catch (error) {
          console.error(
            `[ERROR] Fallo al eliminar la imagen con ID ${imageId}:`,
            error,
          );
      ...
      if (imagesUpdateDto.images.length > 0) {
        const salaActualizadaI =
          await service.instance.updateSalaDeEnsayoImagenes(
            id,
            imagesUpdateDto,
          );
        ...
        if (!salaActualizadaI) {
          return resp.status(500).json({
            status: 500,
            msg: "Error al actualizar las imágenes de la sala.",
          });
        }
      }
      //actualizar data de sala
      const salaActualizada = await service.instance.updateSalaDeEnsayo(
        id,
        dto,
      );
      if (!salaActualizada) {
        return resp
          .status(500)
          .json({ status: 500, msg: "Error al actualizar la sala." });
      }
  ...
  app.post(
    "/salasdeensayo/reportesNuevasSdE",
    auth,
    admin,
    ...
    run(async (req: any, resp: Response) => {
      ...
      const NewUsersReport = await service.instance.reporteNuevasSdE(
        dto.fechaI,
        dto.fechaH,
      );
  ...
  app.post(
    "/salasdeensayo/descargarReportesNuevasSdE",
    auth,
    admin,
    validator.body("fechaI").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
    validator.body("fechaH").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
  ...
  app.post(
    ...
    run(async (req: any, resp: Response) => {
      ...
      const salaOriginal: SalaDeEnsayoDto =
        await service.instance.findSalaById(idRoom);
      ...
      if (!sala) {
        resp
          .status(500)
          .json({ msg: "No se pudo actualizar la sala con la opinion" });
        return;
      }
      // realizar calculos de promedio luego de crear la opinion
      const salaUpdated: SalaDeEnsayoDto =
        await service.instance.findSalaById(idRoom);
      const opinionesIds = salaUpdated.opiniones;
      const opiniones = await OpinionModel.find({ _id: { $in: opinionesIds } });

      const totalEstrellas = opiniones.reduce(
        (total, opinion) => total + opinion.estrellas,
        0,
      );
  ...
  app.post(
    "/salasdeensayo/createOpinionToArtist/",
    auth,
  ...
  app.put(
    ...
    run(async (req: any, resp: Response) => {
      ...
      console.log("ruta update Opinion: ", dto, "id: ", id);
      const opinionOriginal: OpinionDto =
        await service.instance.getOpinionById(id);
      if (!dto["descripcion"]) {
  ...
  app.put(
    "/saladeensayo/updateOpinionToArtist/",
    auth,
  ...
  app.get(
    ...
    run(async (req: Request, res: Response) => {
      const id = req.query.id as string;
      const salaDeEnsayo: SalaDeEnsayoDto =
        await service.instance.findSalaById(id);
  ...
  app.get(
    ...
    run(async (req: Request, res: Response) => {
      ...
      if (!salaDeEnsayo) {
        return res
          .status(404)
          .json({ message: "Sala de ensayo no encontrada" });
      }
      const opinionesIds = salaDeEnsayo.opiniones;
      const opiniones = await OpinionModel.find({ _id: { $in: opinionesIds } })
        .populate("idUser")
  ...
  app.get(
    ...
    run(async (req: Request, res: Response) => {
      ...
      const opiniones: OpinionDto[] = await OpinionModel.find({ idRoom: id })
        .populate("idUser")
        .exec();
  ...
  app.get(
    "/salaOpinion/getMyOpinionToRoom/",
    auth,
    checkArtistOrSalaDeEnsayo,
    validator.query("idRoom").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
    run(async (req: any, resp: Response) => {
      ...
      const opinion = await service.instance.getOpinionByUserAndRoom(
        idUser,
        idRoom,
      );
  ...
  app.get(
    ...
    run(async (req: any, resp: Response) => {
      ...
      //const opinion = await service.instance
      const opinion = await service.instance.getOpinionByUserAndArtist(
        idUser,
        idArtist,
      );
      console.log("route response opinion to room: ", opinion);
  ...
  app.get(
    ...
    run(async (req: Request, resp: Response) => {
      ...
      const id = req.query.id as string;
      const opiniones: OpinionDto[] =
        await service.instance.getOpinionToArtist(id);
      resp.json(opiniones);
    }),
  );
    ...
    run(async (req: Request, resp: Response) => {
      ...
      async function obtenerGrafTortaTipoSala(
        ...
        try {
          ...
          console.error(
            "Error al obtener el gráfico de torta por tipo de sala:",
            error,
          );
          throw error;
        }
      ...
      try {
        const resultados = await obtenerGrafTortaTipoSala(
          fechaInicioObj,
          fechaFinObj,
        );
        ...
        if (resultados.length === 0) {
          ...
          const totalPopulation = resultados.reduce(
  ...
  app.post(
    "/salasDeEnsayo/descargarReporteTipoSalaTorta",
    auth,
    admin,
    validator.body("fechaI").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
    validator.body("fechaH").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
    run(async (req: Request, resp: Response) => {
      ...
      try {
        ...
        //Codigo Javascript :
        const chartImage = await generateReportePieChart(
          formattedResult.labels,
          formattedResult.datasets[0].data,
          " Tipos de Salas de Ensayo",
        ); // Generar el gráfico de barras
        //const chartImageBasic = await generateBarChart(mesesString, arrCantidades); // Generar el gráfico de barras
        const pdfBytes = await generateReportePDF(
          chartImage,
          "Reporte - Tipos de Salas de Ensayo",
          fechaInicio,
          fechaHasta,
        ); // Generar el PDF con el gráfico
  ...
  app.get(
    "/salasdeensayo/cantidadValoraciones/",
    auth,
    run(async (req: any, resp: Response) => {
      const errors = validator.validationResult(req);
      if (errors && !errors.isEmpty()) {
      ...
      const NewUsersReport =
        await service.instance.obtenerCantidadValoracionesDos(id);
  ...
  app.get(
    "/salasdeensayo/descargarCantidadValoraciones/",
    auth,
    run(async (req: any, resp: Response) => {
      try {
        const errors = validator.validationResult(req);
        ...
        const pdfBytes = await generateReporteValoracionesPDF(
          chartImage,
          "Reporte - Cantidad de valoraciones",
          salaNombre,
          fechaInicio,
          fechaHasta,
        ); // Generar el PDF con el gráfico
  ...
  //TODO probar
  app.get(
    "/salasdeensayo/promedioCalificacionSalas/",
    auth,
    run(async (req: any, res: Response) => {
      ...
      if (!myRooms || myRooms.length === 0) {
        console.log(
          "No hay salas de ensayo disponibles para el usuario:",
          idUser,
        );
        res.json({
          status: "success",
      ...
      for (const room of myRooms) {
        ...
        const promedio = totalEstrellas / opiniones.length;
        console.log(
          `Promedio de calificación para la sala ${room.nameSalaEnsayo}: ${promedio}`,
        );
        total += promedio;
      }
  ...
  app.get(
    "/salasdeensayo/opinionesAMisSalas/",
    auth,
    run(async (req: any, res: Response) => {
      const idUser = req.user.id;
      const errors = validator.validationResult(req);
  ...
  app.post(
    "/salasdeensayo/postOpinionASala/",
    auth,
    checkPermission(["CALIFICAR_SALA_DE_ENSAYO"]),
    // validator.body("idRoom").notEmpty().withMessage(ErrorCode.FIELD_REQUIRED),
    validator
    ...
    run(async (req: any, resp: Response) => {
      ...
      const dto = req.body;
      console.log("ruta post opinion a sala idRoom: ", dto["idRoom"]);
      console.log("ruta post opinion a sala, idUser: ", req.user.id);
      ...
      try {
        ...
        const opinion = await service.instance.postOpinion(datosOpinion);
        console.log("Ruta, opinion creada: ", opinion);
        resp.json(opinion);
      } catch (error) {
