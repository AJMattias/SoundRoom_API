...
export class SalaService {
  ...
  async findByNamePaginated(
    ...
    console.log(
      "Service buscando sala por nombre, página:",
      page,
      "límite:",
      limit,
    );
  ...
  /*
    ...
    */
  async createSalaDeEnsayo(
    dto: CreateSalaDeEnsayoDto,
  ): Promise<SalaDeEnsayoDto> {
  ...
  async updateSalaDeEnsayoImagenes(
    id: string,
    dto: UpdateImageDto,
  ): Promise<SalaDeEnsayoDto> {
  ...
  async updateSalaDeEnsayo(
    id: string,
    dto: UpdateSalaDeEnsayoDto2,
  ): Promise<SalaDeEnsayoDto> {
  ...
  async deleteSalaDeEnsayo(
    id: string,
    dto: CreateSalaDeEnsayoDto2,
  ): Promise<SalaDeEnsayoDto> {
    return this.mapToDto(
  ...
  async obtenerCantidadNuevasSdEPorMes(
    fechaInicio: string,
    fechaFin: string,
    ...
    try {
      ...
      for (let i = 0; i < diffMeses; i++) {
        const fechaActual = new Date(
          ...
          fechaInicioObj.getMonth() + i,
          1,
        );
        const fechaSiguiente = new Date(
          fechaInicioObj.getFullYear(),
          fechaInicioObj.getMonth() + i + 1,
          1,
        );
      ...
      console.error(
        "Error al obtener la cantidad de documentos por mes:",
        error,
      );
      throw error;
    }
    ...
    try {
      ...
      sde.forEach((sala) => {
        ...
        const index = labels.findIndex(
          (label) => label === this.getMonthAbbreviation(mes),
        );
        if (index !== -1) {
      ...
      console.log(
        `Salas de ensayo nuevas agrupados por mes: ${JSON.stringify({ labels, data })}`,
      );
  ...
  async obtenerCantidadValoraciones(idRoom: string) {
    try {
      ...
      const opiniones: OpinionDto[] = await OpinionModel.find({
        idRoom: idSala,
      }).exec();
  ...
  async getOpinionToArtist(idArtist: string): Promise<Array<OpinionDto>> {
    ...
    console.log(
      "service promedio estrellas de artista opiniones, get opiniones: ",
      opiniones,
    );
  ...
  async getMyAllOpiniones(idUser: string): Promise<Array<any>> {
    console.log("servicio get salas por owner id: ", idUser);
    const salas = await this.findSalaByOwner(idUser);
    console.log("servicio getallmyopniniones - get salas por owner: ", salas);
    const opiniones = await Promise.all(
      salas.map(async (sala) => {
        const opinionesSala = await this.dao.getOpinionByRoom(
          sala.id as unknown as string,
        );
        return {
          sala: sala,
          opiniones: opinionesSala.map((opinion: Opinion) =>
            this.mapToDtoOpinion(opinion),
          ),
        };
      }),
    );
    console.log("service get opiniones a mis salas: ", opiniones);
    return opiniones;
  }
