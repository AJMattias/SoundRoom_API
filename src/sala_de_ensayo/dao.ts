...
import {
  CreateOpinionArtistDto,
  CreateOpinionDto,
  CreateSalaDeEnsayoDto,
  CreateSalaDeEnsayoDto2,
  CreateSalaDeEnsayoDtoOpinion,
  CreateSearchSdEDto,
  OpinionDto,
  SalaDeEnsayoDto,
  UpdateSalaDeEnsayoDto2,
} from "./dto";
import {
  Opinion,
  OpinionDoc,
...
export class SalaDeEnsayoDao {
  async getAll(): Promise<Array<SalaDeEnsayo>> {
    return (await SalaDeEnsayoModel.find({ enabled: "habilitado" }).exec()).map(
      (doc: SalaDeEnsayoDoc) => {
  ...
  async getByOwner(idOwner: string): Promise<Array<SalaDeEnsayo>> {
    const sala = await SalaDeEnsayoModel.find({
  ...
  async getByOwnerPaginated(
    ...
    const [salas, total] = await Promise.all([
      SalaDeEnsayoModel.find({ idOwner: new mongoose.Types.ObjectId(idOwner) })
        ...
        .limit(limit)
        .exec(),
      SalaDeEnsayoModel.countDocuments({
        idOwner: new mongoose.Types.ObjectId(idOwner),
      }),
    ]);
  ...
  async updateSala(
    salaEnsayoId: string,
    sala: UpdateSalaDeEnsayoDto2,
  ): Promise<SalaDeEnsayo> {
    ...
    const updated = await SalaDeEnsayoModel.findByIdAndUpdate(
      // salaEnsayoId,
      new Types.ObjectId(salaEnsayoId),
      updatePayloadF,
      { new: true },
    ).exec();
  ...
  async disableSala(
    ...
    const updated = await SalaDeEnsayoModel.findByIdAndUpdate(
      salaEnsayoId,
      {
        ...
        $push: {
          enabledHistory: { status: "deshabilitado", dateFrom: new Date() },
        },
      },
      { new: true },
    );
  ...
  async enableSala(
    ...
    const updated = await SalaDeEnsayoModel.findByIdAndUpdate(
      salaEnsayoId,
      {
        ...
        descripcion: sala.descripcion,
        $push: {
          enabledHistory: { status: "habilitado", dateFrom: new Date() },
        },
      },
      { new: true },
    );
  ...
  async updateSalaOpinion(
    salaEnsayoId: string,
    sala: CreateSalaDeEnsayoDtoOpinion,
    idOpinion: string,
  ): Promise<SalaDeEnsayo> {
    const updated = await SalaDeEnsayoModel.findByIdAndUpdate(salaEnsayoId, {
      nameSalaEnsayo: sala.nameSalaEnsayo,
  ...
  async updateSalaImages(
    salaEnsayoId: string,
    imagesToUpdate: UpdateImageDto,
  ): Promise<SalaDeEnsayo> {
  ...
  mapToSalaDeEnsayoImagen(document: SalaDeEnsayoDoc): SalaDeEnsayo {
    const imagenesFormatoCorrectoParaInterfaz = document.imagenes.map((id) => ({
      type: id.toString(),
    }));
  ...
  async getAllOpiniones(): Promise<Array<Opinion>> {
    return (await OpinionModel.find().exec()).map((doc: OpinionDoc) => {
  ...
  async getOpinionByUserAndArtist(
    idUser: string,
    idArtist: string,
  ): Promise<Opinion> {
  ...
  async getOpinionById(opinionId: string): Promise<Opinion> {
	const id = new mongoose.Types.ObjectId(opinionId);
    const model = await OpinionModel.findById(id).exec();
	console.log('dao get opinion by id, model: ', model)
    if (!model) throw new ModelNotFoundException();
  ...
  async getOpinionToArtist(artistId: string): Promise<Array<Opinion>> {
    ...
    return (
      await OpinionModel.find({ idArtist: idArtist }).populate("idUser").exec()
    ).map((doc: OpinionDoc) => {
      return this.mapToOpinion(doc);
    });
  }
  ...
  async getOpinionByRoom(idRoom: string): Promise<Array<Opinion>> {
    ...
    console.log('dao get opinions a mis salas: ', opinionDoc);
    return opinionDto;
  }

  // Opinion.dao.ts
  async createOpinionUpdate(data: Partial<OpinionDto>): Promise<OpinionDoc> {
    // 1. Crear la opinión
    const nuevaOpinion = new OpinionModel({
      ...data,
      createdAt: new Date(),
    });
    const savedOpinion = await nuevaOpinion.save();

    // 2. Vincular la opinión a la sala (Update bidireccional)
    await SalaDeEnsayoModel.findByIdAndUpdate(data.idRoom, {
      $push: { opiniones: savedOpinion._id },
    });
