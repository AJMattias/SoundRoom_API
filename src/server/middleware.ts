...
import { DiasSemana } from "../sala_de_ensayo/model"
...
export const validarDiaSemana = (
  ...
  for (const [index, horario] of horarios.entries()) {
    ...
    if (!Object.values(DiasSemana).includes(dia as DiasSemana)) {
      return res.status(400).json({
        message: `El día "${dia}" no es válido. Los días permitidos son: ${Object.values(
          DiasSemana
        ).join(', ')}.`,
      });
    }
