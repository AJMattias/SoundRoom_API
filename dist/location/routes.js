...
const route = (app) => {
    ...
    app.post("/locality/", validator.body("nameLocality").notEmpty().withMessage(constants_1.ErrorCode.FIELD_REQUIRED), validator.body("idProvince").notEmpty().withMessage(constants_1.ErrorCode.FIELD_REQUIRED), (0, run_1.run)(async (req, resp) => {
        const errors = validator.validationResult(req);
        if (errors && !errors.isEmpty()) {
            throw validator_utils_1.ValidatorUtils.toArgumentsException(errors.array());
        }
    ...
    app.put("/locality/", validator.query("id").notEmpty().withMessage(constants_1.ErrorCode.FIELD_REQUIRED), (0, run_1.run)(async (req, resp) => {
        const errors = validator.validationResult(req);
        if (errors && !errors.isEmpty()) {
            throw validator_utils_1.ValidatorUtils.toArgumentsException(errors.array());
        }
        ...
        const localityOriginal = await service.instance.findLocalityById(id);
        if (!dto["nameLocality"]) {
