...
class LocationDao {
    async getAllLocalities() {
        return (await models_js_1.LocalityModel.find({}).exec())
            .map((doc) => {
    ...
    async findByIdL(localityId) {
        const model = await models_js_1.LocalityModel.findById(localityId).exec();
        if (!model)
            throw new exception_1.ModelNotFoundException();
        return this.mapToLocality(model);
    }
    async storeL(locality) {
        const localityDoc = await models_js_1.LocalityModel.create({
            nameLocality: locality.nameLocality,
            id: locality.id ? string_utils_1.StringUtils.toObjectId(locality.id) : undefined
        });
    ...
    async updateLocality(localityId, locality) {
        const updated = await models_js_1.LocalityModel.findByIdAndUpdate(localityId, {
            nameLocality: locality.nameLocality
        }).exec();
        if (!updated) {
            throw new exception_1.ModelNotFoundException();
